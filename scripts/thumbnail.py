...
def convert_to_images(pptx_path: Path, temp_dir: Path) -> list[Path]:
    """Convert PPTX to individual slide images using render_slides."""
    out_dir = str(temp_dir / "rendered")
    try:
        dpi = render_slides.calc_dpi(str(pptx_path), 1600, 900)
    except Exception:
        dpi = CONVERSION_DPI
    paths = render_slides.rasterize(str(pptx_path), out_dir, dpi)
