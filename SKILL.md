...
Convert slides to images (see [Converting to Images](#converting-to-images)), then look for:
- Small fonts: body text below 14pt or labels below 10pt — reduce content or use spacing to fit, never shrink fonts
- Too much whitespace: large empty regions that make the slide look sparse — tighten spacing between elements rather than spreading them across the full slide
...
slide.addImage({ data: "image/png;base64,iVBOR0KGgo...", x: 1, y: 1, w: 5, h: 3 });
```
